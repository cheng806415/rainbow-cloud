...
case 'folder_list':
	...
	if($islogin2){
		$sql = " uid='{$uid}' AND parent_id={$parent_id}";
	}else{
		$sql = " 1=2";
	}
	$folders = $DB->getAll("SELECT * FROM pre_folder WHERE{$sql} ORDER BY id DESC");
...
case 'folder_setpwd':
	...
	$DB->exec("UPDATE pre_folder SET pwd=:pwd WHERE id=:id", [':pwd'=>$pwd, ':id'=>$folder_id]);
...
case 'folder_create':
	...
	$exist = $DB->getRow("SELECT * FROM pre_folder WHERE name=:name AND parent_id=:parent_id AND uid=:uid", [':name'=>$name, ':parent_id'=>$parent_id, ':uid'=>$uid]);
	if($exist)exit('{"code":-1,"msg":"该文件夹已存在"}');
	$sds = $DB->exec("INSERT INTO `pre_folder` (`name`,`parent_id`,`uid`,`addtime`) values (:name,:parent_id,:uid,NOW())", [':name'=>$name, ':parent_id'=>$parent_id, ':uid'=>$uid]);
...
case 'folder_delete':
	...
	$DB->exec("UPDATE pre_file SET folder_id=0 WHERE folder_id=:id", [':id'=>$folder_id]);
