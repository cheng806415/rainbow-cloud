        ...
        if($current_folder){
            if($current_folder['pwd']){
                ...
                if(!empty($_POST['folder_pwd'])){
                    if(md5($_POST['folder_pwd']) === $current_folder['pwd']){
        ...
        <div class="panel panel-default" id="userFolderList">
            ...
            <div class="table-responsive">
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                    <?php
                    $user_folders = $DB->getAll("SELECT u.uid, u.username, u.nickname FROM pre_user u WHERE u.enable=1 ORDER BY u.uid");
                    ...
                    ?>
                        <tr>
                            <td><a href="?view_mode=folder&folder_id=<?php 
                                $root_folder = $DB->getRow("SELECT id FROM pre_folder WHERE uid=:uid AND hide=0 LIMIT 1", [':uid'=>$uf['uid']]);
                                if($root_folder) echo $root_folder['id'];
                            ?>"><i class="fa fa-folder" style="color:#337ab7;"></i> <?php echo htmlspecialchars($uf['username']?$uf['username']:'用户'.$uf['uid'])?></a></td>
    ...
    $('#newFolderBtn').click(function(){
        layer.prompt({title: '输入文件夹名称', formType: 0}, function(name, index){
            ...
            $.ajax({
                ...
                data: {name: name, parent_id: 0, csrf_token: '<?php echo $csrf_token;?>'},
                dataType: 'json',
                success: function(data){
                    layer.close(ii);
                    if(data.code == 0){
