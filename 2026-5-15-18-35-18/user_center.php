            ...
            <div class="col-md-7">
                <div class="profile-card">
                    ...
                    <div class="settings-section" style="border-top:1px solid #eee;padding-top:20px;">
                        <h4><i class="fa fa-link"></i> 账号绑定</h4>
                        <div class="form-row">
                            <label>微信</label>
                            <span id="wxStatus">
                                <?php if($userrow['wx_openid']){?>
                                <img src="<?php echo htmlspecialchars($userrow['wx_faceimg']?:'')?>" style="width:30px;height:30px;border-radius:50%;vertical-align:middle;margin-right:8px;">
                                <?php echo htmlspecialchars($userrow['wx_nickname']?:'微信账号')?>
                                <button class="btn btn-xs btn-danger" style="margin-left:10px;" onclick="unbindAccount('wx')">解绑</button>
                                <?php }else{?>
                                <button class="btn btn-xs btn-success" onclick="bindAccount('wx')"><i class="fa fa-wechat"></i> 绑定微信</button>
                                <?php }?>
                            </span>
                        </div>
                        <div class="form-row">
                            ...
                            <span id="qqStatus">
                                ...
                                <img src="<?php echo htmlspecialchars($userrow['qq_faceimg']?:'')?>" style="width:30px;height:30px;border-radius:50%;vertical-align:middle;margin-right:8px;">
                                <?php echo htmlspecialchars($userrow['qq_nickname']?:'QQ账号')?>
                                <button class="btn btn-xs btn-danger" style="margin-left:10px;" onclick="unbindAccount('qq')">解绑</button>
                                <?php }else{?>
                                <button class="btn btn-xs btn-info" onclick="bindAccount('qq')"><i class="fa fa-qq"></i> 绑定QQ</button>
                                <?php }?>
                            </span>
                        </div>
                    </div>
...
$(function(){
    ...
    $('#saveNickname').click(function(){
        ...
        $.ajax({
            ...
            data: {action:'nickname', nickname: nickname, csrf_token: currentCsrfToken},
            dataType: 'json',
            success: function(data){
                layer.close(ii);
                if(data.code == 0){
    ...
    $('#savePrivacy').click(function(){
        ...
        $.ajax({
            type: 'POST',
            url: 'ajax.php?act=saveUserSettings',
            data: {action:'privacy', allow_view: allow_view, allow_search: allow_search, csrf_token: currentCsrfToken},
            dataType: 'json',
            success: function(data){
                layer.close(ii);
                if(data.code == 0){
    ...
    $('#changePassword').click(function(){
        ...
        $.ajax({
            type: 'POST',
            url: 'ajax.php?act=saveUserSettings',
            data: {action:'password', old_password: oldPassword, new_password: newPassword, csrf_token: currentCsrfToken},
            dataType: 'json',
            success: function(data){
                layer.close(ii);
                if(data.code == 0){
...
function unbindAccount(type){
    layer.confirm('确定要解绑此账号吗？', {
        ...
        $.ajax({
            type: 'POST',
            url: 'ajax.php?act=unbindAccount',
            data: {type: type, csrf_token: currentCsrfToken},
            dataType: 'json',
            success: function(data){
                layer.close(ii);
                if(data.code == 0){
