...
case 'userList':
	...
	$siteurl = (is_https() ? 'https://' : 'http://').$_SERVER['HTTP_HOST'].substr($sitepath, 0, strrpos($sitepath, '/')).'/';
	
	$list2 = [];
	foreach($list as $row){
		$row['type'] = $type_arr[$row['type']] ?? $row['type'];
		$faceimg = isset($row['faceimg']) && $row['faceimg'] ? $row['faceimg'] : (isset($row['avatar']) && $row['avatar'] ? $row['avatar'] : '');
		if(strpos($faceimg, 'http') !== 0 && strpos($faceimg, './') === 0){
			$faceimg = $siteurl . substr($faceimg, 2);
		}
		$row['faceimg'] = $faceimg;
		$row['file_count'] = $DB->getColumn("SELECT count(*) FROM pre_file WHERE uid=:uid", [':uid'=>$row['uid']]);
		$row['folder_count'] = $DB->getColumn("SELECT count(*) FROM pre_folder WHERE uid=:uid", [':uid'=>$row['uid']]);
		$list2[] = $row;
	}

	exit(json_encode(['total'=>$total, 'rows'=>$list2], JSON_UNESCAPED_UNICODE));
break;
case 'setUserEnable':
...
case 'saveUserInfo':
	...
	if(!$row)exit('{"code":-1,"msg":"当前用户不存在！"}');
	
	$level=intval($_POST['level']);
	$nickname=trim(daddslashes($_POST['nickname']));
	$resetpwd=trim($_POST['resetpwd']);
	
	if(empty($nickname))exit('{"code":-1,"msg":"昵称不能为空"}');
	
	$sql = "UPDATE pre_user SET level='$level', nickname='$nickname' WHERE uid='$uid'";
	if(!empty($resetpwd)){
		if(strlen($resetpwd) < 6)exit('{"code":-1,"msg":"密码长度不能少于6位"}');
		$newpwd = md5($resetpwd);
		$sql = "UPDATE pre_user SET level='$level', nickname='$nickname', password='$newpwd' WHERE uid='$uid'";
	}
	if($DB->exec($sql)!==false)exit('{"code":0,"msg":"修改用户成功！"}');
	else exit('{"code":-1,"msg":"修改用户失败['.$DB->error().']"}');
break;
