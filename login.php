...
if(isset($_GET['logout'])){
    ...
    if(password_verify($password, $userrow['password'])){
        ...
        $DB->exec("UPDATE pre_user SET password=:password WHERE uid=:uid", [':password'=>$new_pwd_hash, ':uid'=>$userrow['uid']]);
    }
    if(!$pwd_correct){
    ...
    if(!$DB->exec("INSERT INTO pre_user (username, password, type, openid, nickname, enable, regip, loginip, addtime, lasttime) VALUES (:username, :password, '', '', :nickname, 1, :regip, :loginip, NOW(), NOW())", [':username'=>$username, ':password'=>$pwd_hash, ':nickname'=>$username, ':regip'=>$clientip, ':loginip'=>$clientip])){
    ...
    if($islogin2){
        $exist_user=$DB->getRow("SELECT uid FROM pre_user WHERE type=:type AND openid=:openid", [':type'=>$type, ':openid'=>$openid]);
        if($exist_user && $exist_user['uid']!=$uid){
            sysmsg('该'.$typename.'账号已绑定其他账号');
        }
        if($type=='wx'){
            $DB->exec("UPDATE pre_user SET wx_openid=:openid, wx_nickname=:nickname, wx_faceimg=:faceimg WHERE uid=:uid", [':openid'=>$openid, ':nickname'=>$nickname, ':faceimg'=>$faceimg, ':uid'=>$uid]);
        }elseif($type=='qq'){
            $DB->exec("UPDATE pre_user SET qq_openid=:openid, qq_nickname=:nickname, qq_faceimg=:faceimg WHERE uid=:uid", [':openid'=>$openid, ':nickname'=>$nickname, ':faceimg'=>$faceimg, ':uid'=>$uid]);
        }
        @header('Content-Type: text/html; charset=UTF-8');
        exit("<script language='javascript'>alert('".$typename."账号绑定成功！');window.location.href='./user_center.php';</script>");
    }
    ...
    ob_clean();
    setcookie("user_token", $token, time() + 2592000, '/');
    exit("<script language='javascript'>window.location.href='./user_center.php';</script>");
}
